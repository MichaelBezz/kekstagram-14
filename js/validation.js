...
(function () {
  ...
  var MAX_SYMBOL_HASHTAG = 20;
  var MAX_QUANTITY_HASHTAGS = 5;
  var MAX_SYMBOL_TEXT_DESCRIPTION = 140;
  ...
  var onFormHashtagsValidityBlur = function () {
    ...
    if (hashtagsValue === '') {
      ...
      textHashtags.classList.remove('message-error-border');
      return;
    }

    if (arrHashtags.length > MAX_QUANTITY_HASHTAGS) {
      textHashtags.setCustomValidity('Максимальное количество хэш-тегов 5');
      textHashtags.classList.add('message-error-border');
      return;
    } else {
      textHashtags.setCustomValidity('');
      textHashtags.classList.remove('message-error-border');
    ...
    arrHashtags.forEach(function (item) {
      if (item === '#') {
        ...
        textHashtags.classList.add('message-error-border');
      } else if (item.length > MAX_SYMBOL_HASHTAG) {
  ...
  var onFormTextDescriptionValidityBlur = function (evtText) {
    ...
    if (target.value.length > MAX_SYMBOL_TEXT_DESCRIPTION) {
      textDescription.classList.add('message-error-border');
      textDescription.setCustomValidity('Длина комментария не может превышать 140 символов');
    } else {
      textDescription.classList.remove('message-error-border');
